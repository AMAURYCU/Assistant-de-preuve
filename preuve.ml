...
let _,f = seq.theoreme in 
  match f with 
  |ApplyDef(s,tl) -> let de =  (Hashtbl.find Proof_ast.table s) in begin match de with |DefForm(a,b) ->
                    [{var = seq.var; hypot = seq.hypot ; theoreme = ("G", (subst_form {identp = ("_"^(List.hd a).identp); sortp =(List.hd a).sortp;}  (List.hd tl)  b)) }]
                    |_-> raise(PreuveInvalide("erreur dans unpack")) end
